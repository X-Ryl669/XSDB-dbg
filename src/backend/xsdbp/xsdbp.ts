...
export class XMI_XSDB extends EventEmitter implements IBackend {
    ...
    findSourceFile(basename: string, address: number = 0) : Promise<IFileSource> {
        try {
            if (fs.lstatSync(basename).isFile())
                return Promise.resolve({file:basename, line: 0});
        } catch(e) {
            // Ignore missing error here
        }
        const basePath = this.targetDir.replace("/Work", "/src");
    ...
    addBreakPoint(breakpoint: Breakpoint): Thenable<[boolean, Breakpoint]> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.sendCommand("bpadd " + args).then((answer) => {
                if (answer.complete) {
                    const bpt = (answer.value as XSDBAddedBreakpoint);
    ...
    async getStack(startFrame: number, maxLevels: number, thread: number): Promise<Stack[]> {
        ...
        for (let i = startFrame; i < highBand; i++) {
            const filepath = {file: "<unknown>", line: frames.frame[i].line, func: frames.frame[i].func};
            if (frames.frame[i].file) {
                const fp = await this.findSourceFile(path.normalize(frames.frame[i].file), frames.frame[i].address);
    ...
    async getStackVariables(thread: number, frame: number): Promise<Variable[]> {
        ...
        try
        {
            ...
            if (answer.complete && answer.value !== undefined) {
                const defsAnswer = await this.sendCommand("locals -defs");
                const defs = defsAnswer.complete && defsAnswer.value !== undefined ? (defsAnswer.value as XSDBLocalsDef).variables : [];
                const variables = answer.value as XSDBLocals;
                for (const element of variables.variables) {
                    const v = this.parseLocalValue(element.value);
        ...
        if (regAnswer.complete && regAnswer.value !== undefined) {
